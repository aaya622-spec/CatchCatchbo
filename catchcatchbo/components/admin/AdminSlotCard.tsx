...
export default function AdminSlotCard({
  ...
  return (
    ...
    >
      ...
      {/* 장소 & 예약 현황 */}
      <div className="flex items-center gap-3 text-sm text-warm-gray-500 flex-wrap">
        <span>
          📍{" "}
          {getLocationLabel(
            slot.location_text
        ...
        <span>
          {slot.booking_count}/
          {slot.max_guests}명 예약
        </span>
      </div>

      {/* 설명 */}
      {slot.description && (
        <p className="text-sm text-warm-gray-400 bg-cream-100 rounded-xl px-3 py-2 leading-relaxed">
          {slot.description}
        </p>
      )}
