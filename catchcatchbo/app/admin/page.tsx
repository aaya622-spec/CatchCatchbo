    ...
    <div className="min-h-screen pb-20">
      ...
      <header className="sticky top-0 z-10 bg-cream-100/90 backdrop-blur-sm border-b border-cream-200 px-5 py-4">
        <div className="flex items-center justify-between">
          <h1 className="font-bold text-warm-gray-800">{APP_NAME} 관리</h1>
          <form action={signOut}>
            <button
              type="submit"
      ...
      <div className="px-5 pt-6 flex flex-col gap-8">
        ...
        <section className="grid grid-cols-3 gap-3">
          <div className="card p-3 text-center">
            <p className="text-2xl font-bold text-peach-400">{activeSlots.length}</p>
            <p className="text-xs text-warm-gray-400 mt-0.5">열려있는 날</p>
          </div>
          <div className="card p-3 text-center">
            <p className="text-2xl font-bold text-sage-400">
              {confirmedBookings.length}
            </p>
            <p className="text-xs text-warm-gray-400 mt-0.5">예약 완료</p>
          </div>
          <div className="card p-3 text-center">
            <p className="text-2xl font-bold text-warm-gray-600">
              {slots.reduce((acc, s) => acc + s.remaining, 0)}
            </p>
            <p className="text-xs text-warm-gray-400 mt-0.5">남은 자리</p>
          </div>
        </section>

        {/* 공유 링크 */}
        <ShareSection bookUrl={bookUrl} />

        {/* 일정 추가 버튼 */}
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-warm-gray-700">내 일정</h2>
            <Link
          ...
          {slots.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              {inactiveSlots.length > 0 && (
                <>
                  <p className="text-xs text-warm-gray-400 mt-2">비활성 일정</p>
        ...
        {/* 예약 현황 */}
        <section>
          <h2 className="font-semibold text-warm-gray-700 mb-4">예약 현황</h2>
          <BookingList bookings={(bookings as Booking[]) ?? []} />
        </section>
      </div>
    </div>
  );
